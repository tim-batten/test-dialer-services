...
export class FilterConfigManager extends GenericEntityManager<FilterDefinition> {
  ...
  async createFilterAtNavient(makeFilterRequest: MakeFilterRequest): Promise<MakeFilterResponse> {
    const headers = {
    ...
    const { body }: any = await this.navientCrudClient.post(makeFilterRequest, headers, https);
    this.validateNavientResponse(body);
  ...
  async add(filter: FilterDefinition) {
    ...
    try {
      makeFilterResponse = await this.createFilterAtNavient(makeFilterRequest);
    } catch (e) {
  ...
  async removeFilterInSql(removeFilterRequest: RemoveFilterRequest): Promise<RemoveFilterResponse> {
    ...
    const { body }: any = await this.navientCrudClient.post(removeFilterRequest, headers, https);
    this.validateNavientResponse(body);
  ...
  async updateFilterInSql(updateFilterRequest: UpdateFilterRequest): Promise<RemoveFilterResponse> {
    ...
    const https = serviceConfig.apiCertificate.getHttpsOptions();
    const { body }: any = await this.navientCrudClient.post(updateFilterRequest, headers, https);
    this.validateNavientResponse(body);

    return body[0] as RemoveFilterResponse;
  }

  private validateNavientResponse(body: any) {
  ...
  public async validateMultiFilter(sequenceFilterValidationRequest: SequenceFilterValidationRequest) {
    ...
    try {
      ...
      const https = serviceConfig.apiCertificate.getHttpsOptions();
      const response = await this.navientCrudClient.post(sequenceFilterValidationRequest, headers, https);
