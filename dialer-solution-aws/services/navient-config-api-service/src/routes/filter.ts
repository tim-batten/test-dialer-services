...
import { getDateFromHHMMString, localIANAZone } from 'navient-common/lib/utils/date-helper';
import { makeAuth } from '../auth/auth';
import { ALL_SERVICE_ROLES } from 'navient-common/lib/types/roles';
...
export function makeFilterRouter(): Router {
  ...
  const navientCrudClient = new RequestClient(serviceConfig.crudApi.crudApiUrl);
  ...
  router.post('/filters/validate', async (req: any, res: Response) => {
    ...
    try {
      ...
      const { body }: any = await navientCrudClient.post(validateFilterRequest, headers, https);
