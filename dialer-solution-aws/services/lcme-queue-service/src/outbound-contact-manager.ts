...
import { ExecutableRecord, QueueDb } from 'lcme-services-common/lib/db/queue-db';
import { setTimeoutLoop } from 'lcme-services-common/lib/utils/general';
...
import { ConnectInstanceService } from 'lcme-services-common/lib/utils/connect-instance-service';
...
import { HistoricalStatsSettableKey } from 'lcme-common/lib/types/stats/historical-stats';
...
export class OutboundContactManager {
  ...
  async executeFlow(
    ...
    try {
      ...
      if (serviceConfig.dev.skipFlowExecution) {
        /**
         * Simulate external stat events being received
         */
        const timeToQueue = Math.floor(Math.random() * serviceConfig.dev.simulatedCallDuration);
        const timeToConnect = Math.floor(Math.random() * (serviceConfig.dev.simulatedCallDuration - timeToQueue)) + timeToQueue;
        const timeToDisconnect = serviceConfig.dev.simulatedCallDuration;
        const simulatedEndStates: HistoricalStatsSettableKey[] = [
          'disconnected_from_agent',
          'abandoned_ivr',
          'abandoned_queue',
        ];
        const endState = simulatedEndStates[Math.floor(Math.random() * simulatedEndStates.length)];
        if (endState === 'disconnected_from_agent') {
          setTimeout(() => {
            this.historicalStatsDb.setContactStats(
              campaignExecution.campaign,
              ['connected_to_agent'],
        ...
        if (endState === 'abandoned_queue' || endState === 'disconnected_from_agent') {
          setTimeout(() => {
            this.historicalStatsDb.setContactStats(campaignExecution.campaign, ['queued'], startDate, contactId, 'external');
          }, timeToQueue);
        }
        setTimeout(() => {
          this.historicalStatsDb.setContactStats(campaignExecution.campaign, endState, startDate, contactId, 'external');
          this.removePendingCall(campaignExecution.id, cacheRecord);
        }, timeToDisconnect);
      }
      return contactId;
