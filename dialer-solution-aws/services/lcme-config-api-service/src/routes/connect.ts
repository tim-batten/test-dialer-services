...
import { listCommandRunner } from 'lcme-services-common/lib/utils/connect-utils';
...
// import { getConnectClient } from 'lcme-services-common/lib/utils/connect-client-helper';
...
export function makeConnectRouter() {
  ...
  router.get('/connect/contact-flows', async (req, res) => {
    ...
    try {
      const flowTypes = typeof req.query.flowTypes === 'string' ? req.query.flowTypes.split(',') : undefined;
  ...
  router.get('/connect/queues', async (req, res) => {
    const { connectClient, connectConfig } = await connectServiceManager.get();
    const queueTypes = typeof req.query.queueTypes === 'string' ? req.query.queueTypes.split(',') : undefined;
    try {
