import { DispositionType } from 'lcme-common/lib/models/outbound-contact-info';
...
import { ContactEvent, ContactEventType } from 'lcme-services-common/lib/types/amazon-connect/contact-event';
import { DbInstanceManager } from 'lcme-services-common/lib/utils/db-instance-manager';
import { WorkBatcher } from 'lcme-services-common/lib/utils/work-batcher';
...
export class ContactEventHandler {
  ...
  async handleEvents(contactEvents: ContactEvent[]) {
    if (contactEvents.length > 0) {
      await Promise.allSettled([this.generateStats(contactEvents), this.handleDispositions(contactEvents)]);
    }
  }
  ...
  async handleDispositions(contactEvents: ContactEvent[]) {
    const dispositionEvents = contactEvents.filter((contactEvent) => contactEvent.eventType === 'DISCONNECTED');
    const contactInfos = await this.redisClientPool.runForcePipeline(async (pipeline) => {
      const promises = Promise.all(
        dispositionEvents.map((contactEvent) =>
