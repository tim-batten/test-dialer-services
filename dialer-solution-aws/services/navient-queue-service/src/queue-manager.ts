...
import {
  ...
  PacingStats,
} from 'navient-common/lib/types/stats/pacing-stats';
...
import { mapToObj, removeEmpties, setTimeoutLoop } from 'navient-services-common/lib/utils/general';
    ...
    await setTimeoutLoop(
      async () => {
        const monitoredExecutionIds = (await this.serviceManager.getJobs()).filter((execId) => {
          if (!execId.includes('campaign_execution')) {
            return false;
          }
