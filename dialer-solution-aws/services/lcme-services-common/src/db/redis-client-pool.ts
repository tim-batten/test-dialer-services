...
import {
  GenericClientPool,
  GenericPoolFactory,
} from "../utils/generic-client-pool";
...
export class RedisClientPool extends GenericClientPool<Redis> {
        ...
        if (execIfPipelineCreated && newPipeline.length > 0) {
          newPipeline.exec().then((result) => {
            ...
            if (scanResult.length > 0) {
              this.logger.log('warn', `Detected pipeline result error: ${scanResult.join(',')}`);
            }
          });
        }
        return toReturn;
  ...
  async zrangebyscore(key: RedisKey, min: ZScoreOpt, max: ZScoreOpt, pipeline?: ChainableCommander) {
    return await this.runP((redisClient) => {
      return new Promise<string[]>((resolve, reject) =>
        redisClient.zrangebyscore(key, makeZScore(min), makeZScore(max), (error, value) =>
          error === null ? resolve(value!) : reject(error)
        )
      );
    }, pipeline);
    ...
    return await this.runP((redisClient) => {
      return new Promise<[string, number][]>((resolve, reject) =>
        redisClient.zrangebyscore(key, makeZScore(min), makeZScore(max), 'WITHSCORES', (error, value) =>
          error === null ? resolve((value || []).reduce((acc, cur, idx) => {
            if ((idx & 1) === 0) {
              acc.push([cur, parseInt(value![idx + 1])]);
            }
            return acc;
          }, [] as [string, number][])) : reject(error)
        )
      );
    }, pipeline);
  }
  ...
  public async jsonDel(key: string, pipeline?: ChainableCommander) {
    return this.call('JSON.DEL', [key], pipeline);
  }
...
export class RequiredEntitiesDoNotExistError extends Error {
  constructor(public readonly missingEntities: string[]) {
    super(
      `The following required entities are missing: ${missingEntities.join(
        ", "
      )}`
    );
  }
