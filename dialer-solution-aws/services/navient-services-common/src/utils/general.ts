...
export async function setTimeoutLoop(
  ...
  const handlerActual = async () => {
    if (forceStop) {
      logger.log('verbose', `setTimeoutLoop: Stopping "${name}"`);
      return;
    }
    ...
    timeoutId = setTimeout(handlerActual, adjustedTimeout);
  };
  if (immediate) {
    await handlerActual();
  } else {
    lastRunTime += timeout;
    timeoutId = setTimeout(handlerActual, immediate ? 0 : timeout);
  }
  return {
    stop: () => {
      logger.log('verbose', `setTimeoutLoop: Stopping "${name}"`);
...
export type TimeoutLoop = Awaited<ReturnType<typeof setTimeoutLoop>>;
