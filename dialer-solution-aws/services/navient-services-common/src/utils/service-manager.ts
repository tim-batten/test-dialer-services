...
export class ServiceManager {
  ...
  public constructor(
    ...
    public readonly serviceInfoDb: ServiceInfoDb,
  ...
  public async start() {
    await setTimeoutLoop(
      ...
      'Service heartbeat',
      this.clusterConfig.serviceHeartbeatFrequency,
      {
        immediate: false,
      }
    );
    ...
    await setTimeoutLoop(
      async () => {
        ...
        const [allServices, workerToJobs] = await this.redisClientPool.runForcePipeline((pipeline) =>
          Promise.all([this.getAllServicesWithJobs(pipeline), this.jobManager.jobDb.getWorkerToJobs(pipeline)])
        );
        ...
        for (const [worker, jobs] of Array.from(workerToJobs)) {
          if (
            !worker.startsWith(`${this.serviceInfoDb.baseKey}:${this.serviceInfo.type}:`) ||
            allActiveServiceKeys.has(worker) ||
            allInactiveServiceKeys.has(worker)
          ) {
            continue;
          }
          ...
          this.logger.log(
            'info',
            `Detected non-existent service ${worker} with jobs; adding to inactive list to redistribute`
          );
        ...
        if (becomeInactive.length > 0) {
          if (this.logger.isLoggable('verbose')) {
            this.logger.log(
              'verbose',
              `The following service instances have become inactive: ${becomeInactive.map(
                (service) => `${service.id} with ${service.jobs.length} jobs`
              )}`
            );
          }
        ...
        if (becomeActive.length > 0) {
          if (this.logger.isLoggable('verbose')) {
            this.logger.log(
              'verbose',
              `The following service instances have become active: ${becomeActive.map((service) => service.id)}`
            );
          }
      ...
      this.clusterConfig.serviceStatusMonitorFrequency,
      {
        immediate: true,
      }
    );
  ...
  public async whileJob(
    jobKey: string,
    name: string,
    timeout: number,
    callback: (
      /** Flag to let the callback know if this is the first run since aquiring the job */
      firstRun: boolean
    ) => Promise<boolean>
  ) {
    let firstRun = true;
    await this.acquireJob(jobKey).catch((e) => {
      this.logger.log('info', `Failed to acquire job ${jobKey} for ${name}: ${e.message}`);
    });
    return setTimeoutLoop(
      async () => {
        if (!(await this.hasJob(jobKey))) {
          return true;
        }
        const result = await callback(firstRun);
        firstRun = false;
        return result;
      },
      name,
  ...
  public async getAllServicesWithJobs(pipeline?: ChainableCommander) {
    return await ServiceManager.getAllServicesWithJobs(
      this.serviceInfoDb,
      this.jobManager.jobDb,
  ...
  public static async getAllServicesWithJobs(
    serviceInfoDb: ServiceInfoDb,
    jobDb: JobManagerDb,
    ...
    const allIds = [
      ...allServices.active.map((service) => service.id),
      ...allServices.inactive.map((service) => service.id),
    ];
