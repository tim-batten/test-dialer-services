...
export class OutboundContactInfo {
  ...
  getRingingDuration(): number | null {
    if (!this.startTime) {
      return null;
    }
    return (this.connectedTime?.getTime() || Date.now()) - this.startTime.getTime();
  }
  ...
  static from(plain: any): OutboundContactInfo {
    return transformAndValidateSync(OutboundContactInfo, plain) as OutboundContactInfo;
  }
}
export type OutboundContactInfoObject = Omit<OutboundContactInfo, 'getRingingDuration' | 'isRinging'>;