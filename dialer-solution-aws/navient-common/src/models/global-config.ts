...
import { IsInt, IsString, IsInstance, IsOptional, Min, ValidateNested, IsNumber } from 'class-validator';
...
export class DialerDefaultsDefinition extends Transformable {
  ...
  /**
   * The maximum amount of time (in seconds) that a call can ring before being disconnected by
   * the dialer
   */
  @IsNumber()
  @Min(0)
  MaxRingTime: number = 0;
}
