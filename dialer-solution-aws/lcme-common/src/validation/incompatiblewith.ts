// Initially has this lib as a dependency however it depended on a version of class-validator
// with crtitical severity vulnerabilities and hasn't been updated
// Source of this code is from:
// https://github.com/alireza-bonab/incompatible-with
...
class IsNotSiblingOfConstraint implements ValidatorConstraintInterface {
  ...
  defaultMessage(args: ValidationArguments) {
    return `${args.property} cannot exist alongside the following defined properties: ${this.getFailedConstraints(
      args
    ).join(', ')}`;
  }
...
function IsNotSiblingOf<T extends { [key: string]: unknown }>(props: string[], validationOptions?: ValidationOptions) {
...
function incompatibleSiblingsNotPresent(incompatibleSiblings: string[]) {
  return function <T extends { [key: string]: unknown }, K extends keyof T>(o: T, v: K) {
    return Boolean(
...
export function IncompatibleWith(incompatibleSiblings: string[]) {
  ...
  return function <T = unknown>(target: T, key: string) {
    ...
    validateIf(target as unknown as { [key: string]: unknown }, key);
  };
}